...
tileblaster.prototype.server = function(){
	...
	self.srvr = http.createServer(function (req, res) {
		...
		self.tile(t.mapid, t.z, t.x, t.y, t.r, t.ext, function(err, stream, meta){
			if (err) return debug("<server> [%s] error: %s", t.p, err), (!!self.config.hints&&res.setHeader("x-err-hint",err.toString())), res.statusCode = 204, res.end();
...
tileblaster.prototype.tile = function(mapid, z, x, y, r, e, fn){
	...
	self._checktile(mapid, z, x, y, r, e, function(err){
		...
		if (!!self.errcache[tilefile] && self.errcache[tilefile] > (Date.now()-self.config.maps[mapid].expires)) return fn(new Error("Known bad tile"), null);
