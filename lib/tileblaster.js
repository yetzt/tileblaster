...
try { var optipng = require("optipng"); } catch (err) { var pnck = null; }
...
tileblaster.prototype.optimize = function(mapid, ext){
	...
	if (self.maps[mapid].optimize) switch (ext) {
		case "png": 
			...
			// return strm.pipe(new optipng(['-zc8','-zm8','-f5','-quiet']));
			return strm.pipe(new optipng(['-o7','-zc8','-zm8','-f5','-quiet','-fix']));
		break;
		case "jpg": 
		case "jpeg": 
			if (!jpck) break;
