...
tileblaster.prototype.heartbeat = function(){
	...
	if (!!self.config.heartbeat) self.statistics = { hits: 0, errs: 0, served: 0 }, self.nsa = require("nsa")({
		...
		setInterval(function(){
			self.nsa.send(self.statistics);
		},300000).unref();
		this.send(self.statistics);
	});
...
tileblaster.prototype.optimize = function(mapid, ext){
	...
	if (self.maps[mapid].optimize) switch (ext) {
		case "png": 
			...
			strm = strm.pipe(new pnck(['-zc8','-zm8','-f5']));
		break;
		case "jpg": 
		case "jpeg": 
			if (!jpck) break;
